    ...
    try {
        ...
        if ($oIfxA->NumFilas() > 0) {
                ...
                do {
                    ...
                    $valor_neto = $valor_compra - $valor_recidual;
                    $vida_util_meses_calc = $vida_util_meses;
                    $valor_repr = ($vida_util_meses_calc > 0) ? round($valor_neto / $vida_util_meses_calc, 2) : 0;
                    $activo_invalido = false;
                    ...
                    while ($periodo_actual <= $fecha_fin_rango) {
                        ...
                        try {
                            ...
                            if ($fin_activo_mes && $mes_inicio > $fin_activo_mes) {
                                ...
                                if ($valor_repr <= 0) {
                                    $estado = 'ERROR';
                                    $motivo = 'Activo ' . $clave_activo . ' procesado sin valor de depreciación. Mes ' . str_pad($mes_iter, 2, '0', STR_PAD_LEFT) . ' omitido por cálculo inválido';
                                    $total_errores++;
                                    $total_invalidos++;
                                    $activo_invalido = true;
                                } else {
					...
					";
                                    ...
                                    $sql_cdep = "INSERT into saecdep (cdep_cod_acti, cdep_cod_tdep,     cdep_mes_depr, cdep_ani_depr, 
                                                     ...
                                                     cdep_gas_depn, cdep_est_cdep,      cdep_fec_cdep, cdep_val_repr )
					                        values ($codigo_activo, '$tipo_depreciacion', $mes_iter,           $anio_iter, 
                                                    '$fecha_hasta',  $empresa,            $sucursal,      $valor_acumulado , 
                                                    $valor_mesual,      'PE',           '$fechaServer',    $valor_repr)";
                                    if (!$oIfx->QueryT('BEGIN')) {
                                        throw new Exception("Error al iniciar transacción de depreciación.");
                                    }
                                    if (!$oIfx->QueryT($sql_cdep)) {
                                        $oIfx->QueryT('ROLLBACK');
                                        throw new Exception("Error al ejecutar consulta de depreciación.");
                                    }
                                    $oIfx->QueryT('COMMIT');
                                    $estado = 'PROCESADO';
                                    $motivo = 'Depreciación creada';
                                    if ($valor_repr <= 0) {
                                        $estado = 'ERROR';
                                        $motivo = 'Activo ' . $clave_activo . ' procesado sin valor de depreciación';
                                        $total_errores++;
                                        $total_invalidos++;
                                        $activo_invalido = true;
                                    }
                ...
                foreach ($audit_log as $fila) {
                    if ($fila['estado'] === 'PROCESADO') {
                        $clase_estado = 'label label-success';
                    } elseif ($fila['estado'] === 'ERROR') {
                        $clase_estado = 'label label-danger';
                    } else {
                        $clase_estado = 'label label-warning';
                    }
                    $tabla_detalle .= '<tr>'
                ...
                $resumen_html = '<div class="row">'
                    ...
                    . '<p><strong>Errores:</strong> ' . $total_errores . '</p>'
                    . '<p><strong>Meses con valor NULL/0:</strong> ' . $total_invalidos . '</p>'
                    . '</div>'
                    . '</div>'
                ...
                if ($total_invalidos > 0) {
                    $oReturn->alert('El proceso finalizó con errores. Revise el resumen.');
                } elseif ($total_errores > 0) {
                    $oReturn->alert('El proceso finalizó con errores. Revise el resumen.');
                } elseif ($total_procesados === 0) {
                    $oReturn->alert('No se generaron depreciaciones porque todos los meses ya existían o están fuera de vida útil.');
                } elseif ($total_omitidos > 0) {
                    $oReturn->alert('Algunos meses fueron procesados y otros omitidos. Revise el detalle.');
                }
            } else {
                $resumen_html = '<div class="row">'
                    . '<div class="col-md-12">'
                    ...
                    . '<p><strong>Errores:</strong> 0</p>'
                    . '<p><strong>Meses con valor NULL/0:</strong> 0</p>'
                    . '</div>'
                    . '</div>'
                    . '<div class="table-responsive" style="max-height: 300px; overflow: auto;">'
                    . '<table class="table table-bordered table-condensed">'
